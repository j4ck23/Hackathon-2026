...
def dropdown():
    ...
    options = [{"value": "ALL", "label": "All Crops"}] + [{"value": c, "label": c} for c in crops]# Create a list of dictionaries with "value" and "label" keys for each crop
...
def runModelGlobal():
    ...
    predictor_string = request.args.get("prediction")
    predictor_list = [p for p in predictor_string.split(",") if p]
    ...
    if predictor in features:
        features.remove(predictor) #checks that the predictor column is not in the features
    ...
    for f in features:
        if f in ["Area", "Crop"]:
            features_lstm.extend([col for col in train_df_lstm.columns if col.startswith(f + "_")])
        else:
            features_lstm.append(f)
    features_lstm = [ f for f in features_lstm if f != predictor and not f.startswith(predictor + "_")] #Remove the predictor column if in features
    ...
    y_train_raw = train_df_lstm[predictor].values.reshape(-1, 1) # Extract the target variable values for the training set as a NumPy array and reshape to be a 2D array with one column
    y_test_raw  = test_df_lstm[predictor].values.reshape(-1, 1)
...
#------------------------------------------Linear Regression model----------------------------------------------------------------------------
    ...
    y_train_lin = train_df_lstm[predictor]

    X_test_lin = test_df_lstm[features_lstm].values
    y_test_lin = test_df_lstm[predictor]
